...
HRESULT CDXUTDialog::AddControl( CDXUTControl* pControl )
{
  ...
  if( FAILED(hr) ) {
    std::cerr << "CDXUTDialog::InitControl" << std::endl;
    return hr;
  }
  ...
  if( FAILED(hr) )
  {
    std::cerr << "CGrowableArray::Add" << std::endl;
    return hr;
  }
...
bool CDXUTDialog::OnCycleFocus( bool bForward )
{
  ...
  if( s_pControlFocus == NULL )
  {
    ...
    std::cerr << "CDXUTDialog: Multiple dialogs are improperly chained together" << std::endl;
    return false;
}
...
HRESULT CDXUTDialogResourceManager::CreateTexture( UINT iTexture )
{
  ...
  if( !pTextureNode->bFileSource )
  {
    if( pTextureNode->nResourceID == 0xFFFF && pTextureNode->hResourceModule == (HMODULE) 0xFFFF )
    {
      ...
      if( FAILED(hr) ) {
        std::cerr << "D3DXCreateTextureFromFileInMemoryEx" << std::endl;
        return hr;
      }
    }
    else
    {
      ...
      if( FAILED(hr) ) {
        std::cerr << "D3DXCreateTextureFromResourceEx" << std::endl;
        return hr;
      }
    }
  }
  else
  {
    ...
    if( FAILED(hr) )
    {
      std::cerr << "DXUTFindDXSDKMediaFileCch" << std::endl;
      return hr;
    }
    ...
    if( FAILED(hr) )
    {
      std::cerr << "D3DXCreateTextureFromFileEx" << std::endl;
      return hr;
    }
  }
...
HRESULT CDXUTComboBox::AddItem( const WCHAR* strText, void* pData )
{
  ...
  if( pItem == NULL )
  {
    std::cerr << "AddItem: E_OUTOFMEMORY" << std::endl;
    return E_OUTOFMEMORY;
  }
...
void* CDXUTComboBox::GetItemData( const WCHAR* strText )
{
  ...
  if( pItem == NULL )
  {
    std::cerr << "CGrowableArray::GetAt" << std::endl;
    return NULL;
  }
